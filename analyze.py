...
async def main():
    ...
    print(f"[{ts}] APK Security Analyzer")
    ...
    with tempfile.TemporaryDirectory(prefix="apkanalyze_") as tmp:
        ...
        # ── 1. Run tools ──────────────────────────────────────────────────
        _log("1/4", "Running tools (apkleaks, jadx, mobsf)…")
        ...
        print(f"       ↳ snippets: {len(ctx.snippets)} interesting patterns")

        # ── 3. AI investigation ───────────────────────────────────────────
        _log("3/4", f"Investigating with {args.model}…")
        from analyst import investigate
        analysis_md = investigate(ctx, workdir, args.model)
    ...
    # Surface risk level from report header
